...
export const createWall = (
    ...
    const delay = (Math.max(MAX_COLS, MAX_ROWS) / 6) * SPEEDS.find((s) => s.value === speed)!.value - 1;
                ...
                for (let col = 0; col < MAX_COLS; col++) {
                    if (row % 2 === 0 || col % 2 === 0) {
                        if (!isRowColEqual(row, col, startTile) && (!isRowColEqual(row, col, endTile))) {
                            setTimeout(() => {
                                document.getElementById(`${row}-${col}`)!.className = `${WALL_TILE_STYLE} animate-wallTraversed`
                            }, delay * col
